...
import { framesToSpriteSheet } from "@/lib/video-sprite-sheet";
...
// keying/reassembling them into a sprite sheet.
//
...
// avoiding a second request that would have to re-send the full sprite
// sheet over the wire, which is what caused the earlier 413 bug.
export async function GET(req: NextRequest) {
  try {
    ...
    if (job.status === "completed") {
      ...
      const imageDataUrl = await framesToSpriteSheet(frames);
